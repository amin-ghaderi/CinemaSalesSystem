...
public static class CinemaSalesDbContextSeed
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Do not mix EnsureCreated with Migrate on relational providers.
        ...
        // Wall-clock times are defined in CEST; PostgreSQL (Npgsql) requires UTC for timestamptz.
        static DateTimeOffset At(int year, int month, int day, int hour, int minute)
        {
            var local = new DateTimeOffset(year, month, day, hour, minute, 0, WorkshopTimeZoneOffset);
